 ...
 *
 * Espressif "How to get CSI" §4.2 — CSI between devices on the same AP.
 * Both STAs join the router and ping the gateway. The sense board measures
 * CSI from frames transmitted by the peer STA (promiscuous + MAC filter).
 */
#include <stdio.h>
#include <string.h>
#include <stdlib.h>
...
/* Fixed peer STA MAC so the sense board can filter without discovery. */
static const uint8_t CONFIG_CSI_PEER_MAC[6] = {0x1a, 0x00, 0x00, 0x00, 0x00, 0x0a};

static const char *TAG = "csi_between";
...
static void wifi_csi_init_from_peer(void)
{
#if CONFIG_IDF_TARGET_ESP32C5 || CONFIG_IDF_TARGET_ESP32C61
...
#endif
    /* Overhear peer STA → AP frames (not only frames addressed to us). */
    ESP_ERROR_CHECK(esp_wifi_set_promiscuous(true));
    ESP_ERROR_CHECK(esp_wifi_set_csi_config(&csi_config));
    ESP_ERROR_CHECK(esp_wifi_set_csi_rx_cb(wifi_csi_rx_cb, (void *)CONFIG_CSI_PEER_MAC));
    ESP_ERROR_CHECK(esp_wifi_set_csi(true));
    ESP_LOGI(TAG, "CSI filter peer MAC " MACSTR, MAC2STR(CONFIG_CSI_PEER_MAC));
}
...
#if CONFIG_CSI_BETWEEN_ROLE_PEER
static esp_err_t apply_peer_mac(void)
{
    ESP_ERROR_CHECK(esp_wifi_disconnect());
    ESP_ERROR_CHECK(esp_wifi_stop());
    ESP_ERROR_CHECK(esp_wifi_set_mac(WIFI_IF_STA, CONFIG_CSI_PEER_MAC));
    ESP_LOGI(TAG, "peer role: local MAC set to " MACSTR, MAC2STR(CONFIG_CSI_PEER_MAC));
    ESP_ERROR_CHECK(esp_wifi_start());
    ESP_ERROR_CHECK(esp_wifi_connect());

    /* Wait until we have an IP again after MAC change. */
    for (int i = 0; i < 50; i++) {
    ...
    ESP_LOGW(TAG, "waiting for IP after MAC change timed out; continuing");
    return ESP_OK;
}
#endif
...
#if CONFIG_CSI_BETWEEN_ROLE_PEER
    ESP_LOGI(TAG, "role=PEER (traffic source)");
    apply_peer_mac();
    wifi_ping_router_start();
    ESP_LOGI(TAG, "pinging gateway; sense board should see CSI from " MACSTR,
             MAC2STR(CONFIG_CSI_PEER_MAC));
#else
    ESP_LOGI(TAG, "role=SENSE (CSI from peer)");
    uint8_t local_mac[6];
    ESP_ERROR_CHECK(esp_wifi_get_mac(WIFI_IF_STA, local_mac));
    ESP_LOGI(TAG, "local MAC " MACSTR, MAC2STR(local_mac));
    wifi_csi_init_from_peer();
    wifi_ping_router_start();
#endif
}
